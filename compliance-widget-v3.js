// COMPLIANCE DASHBOARD WIDGET V3 - With Legend and Bug Fixes
// Include this on each portal page and admin dashboard

function initComplianceWidget(orgName, showAllOrgs = false) {
    console.log('🚀 Initializing Compliance Widget V3 for:', orgName, 'Show All:', showAllOrgs);
    ...
    function loadData() {
        ...
        console.log('📊 Loaded compliance data:', Object.keys(complianceData).length, 'items');
    }
    ...
    function saveData(aircraft) {
        ...
        console.log('💾 Saved data for', aircraft, 'to org:', targetOrg);
    }
    ...
    function renderDashboard() {
        ...
        if (!aircraft || aircraft.length === 0) {
            console.error('❌ No aircraft found for org:', orgName);
            document.getElementById('complianceWidget').innerHTML = '<div style="color: #E63946; padding: 20px;">No aircraft configured for this organization. Please contact admin.</div>';
            return;
        }
        
        let html = '<div class="compliance-widget">';
        html += '<h2 style="color: var(--success); margin-bottom: 10px;">📋 Aircraft Compliance Dashboard</h2>';
        
        // ADD LEGEND
        html += '<div class="compliance-legend">';
        html += '<div class="legend-title">Status Legend:</div>';
        html += '<span class="legend-item"><span class="comp-badge comp-expired">2024-12-01</span> Expired / ≤10 days</span>';
        html += '<span class="legend-item"><span class="comp-badge comp-warning">2025-03-15</span> 11-30 days</span>';
        html += '<span class="legend-item"><span class="comp-badge comp-caution">2025-04-20</span> 31-60 days</span>';
        html += '<span class="legend-item"><span class="comp-badge comp-ok">2025-12-25</span> >60 days</span>';
        html += '<span class="legend-item"><span class="comp-badge comp-permanent">Permanent</span> Never expires</span>';
        html += '<span class="legend-item"><span class="comp-badge comp-missing">not listed</span> No data</span>';
        html += '</div>';
    ...
    function getAircraftOrg(registration) {
        if (registration.includes('BGM') || registration.includes('BGO') || registration.includes('CGO') || registration.includes('KGM')) {
            ...
            return 'McCormick';
        } else if (registration.includes('AAM')) {
            return 'Highveld';
        }
        return 'Unknown';
    }
...
function initCrewComplianceWidget() {
    ...
    function renderCrewDashboard() {
        let html = '<div class="compliance-widget">';
        html += '<h2 style="color: var(--success); margin-bottom: 10px;">👥 Crew Compliance Dashboard</h2>';
